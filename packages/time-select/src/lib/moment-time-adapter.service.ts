...
export class MomentTimeAdapter extends TimeAdapter<Moment> {
  ...
  createTime(hour: number = 0, minute: number = 0, second: number = 0): Moment {
    ...
    if (second < 0 || second > 59) {
      throw new Error(
        `Invalid second ${second}. Second has to be between 0 and 59.`,
      );
    }
  ...
  parse(value: unknown, parseFormat: string): Moment | null {
  ...
  getDayOfWeekNames(_style: "long" | "short" | "narrow"): string[] {
    return [];
  }
  ...
  getNumDaysInMonth(_date: Moment): number {
    return 0;
  }

  getYearName(_date: Moment): string {
    return "";
  }
}

  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => MatTimeSelectInputDirective),
      multi: true,
    },
    {
    ...
    },
    {
      ...
      useExisting: MatTimeSelectInputDirective,
    },
  ],
  exportAs: "matTimeSelectInput",
})
export class MatTimeSelectInputDirective<D>
  implements OnDestroy, ControlValueAccessor, Validator
{
  ...
  @Output() readonly timeChange: EventEmitter<MatTimeSelectInputEvent<D>> =
    new EventEmitter<MatTimeSelectInputEvent<D>>();
  /** Emits when an `input` event is fired on this `<input>`. */
  @Output() readonly timeInput: EventEmitter<MatTimeSelectInputEvent<D>> =
    new EventEmitter<MatTimeSelectInputEvent<D>>();
  ...
  private _onChange: (value: D | null) => void = () => {};
  ...
  set matTimeSelect(value: MatTimeSelectComponent<D>) {
    ...
    this._timeSelectSubscription = this._timeSelect.selectedChange.subscribe(
      (selected: D) => {
        this.value = selected;
        this._onChange(selected);
        this._onTouched();
        this.timeInput.emit(
          new MatTimeSelectInputEvent(this, this._elementRef.nativeElement),
        );
  ...
  get _ariaHasPopup(): boolean {
    return true;
  }

  /** @docs-private */
  @HostBinding("attr.aria-owns")
  get _ariaOwns(): boolean | null {
    return (
      (this._timeSelect && this._timeSelect.opened && !!this._timeSelect.id) ||
      null
    );
  }
  ...
  get _minTime(): string | null {
    return this.min ? this._timeAdapter.toIso8601(this.min) : null;
  }

  /** @docs-private */
  @HostBinding("attr.max")
  get _maxTime(): string | null {
    return this.max ? this._timeAdapter.toIso8601(this.max) : null;
  }
  ...
  constructor(
    ...
    this._parseValidator = (): ValidationErrors | null => {
      return this._lastValueValid
        ? null
        : {
            matTimeSelectParse: { text: this._elementRef.nativeElement.value },
          };
    };
    ...
    this._minValidator = (
      ...
      const value = this._getValidDateOrNull(
        this._timeAdapter.deserialize(control.value),
      );
      return !this.min ||
        !value ||
        this._timeAdapter.compareTime(this.min, value) <= 0
        ? null
        : { matTimeSelectMin: { min: this.min, actual: value } };
    };
    ...
    this._maxValidator = (
      ...
      return !this.max ||
        !value ||
        this._timeAdapter.compareTime(this.max, value) >= 0
        ? null
        : { matTimeSelectMax: { max: this.max, actual: value } };
    };
  ...
  onInput(value: string): void {
    let time = this._timeAdapter.parse(
      value,
      this._timeFormats.parse.timeInput,
    );
  ...
  private _formatValue(value: D | null) {
    this._elementRef.nativeElement.value = value
      ? this._timeAdapter.format(value, this._timeFormats.display.timeInput)
      : "";
  }

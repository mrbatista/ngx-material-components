...
export const MAT_TIME_SELECT_DATA = new InjectionToken<Moment>(
  "mat-time-select-data",
);

/** Injection token that determines the scroll handling while the time select is opened. */
export const MAT_TIME_SELECT_SCROLL_STRATEGY = new InjectionToken<
  () => ScrollStrategy
>("mat-time-select-scroll-strategy");

/** @docs-private */
export function MAT_TIME_SELECT_SCROLL_STRATEGY_FACTORY(
  overlay: Overlay,
): () => ScrollStrategy {
...
export class MatTimeSelectContentComponent<D>
  extends _MatTimeSelectContentMixinBase
  implements CanColor
{
  /** @docs-private */
  ...
  @Input() minTime: Moment | null;
  /** The minimum selectable time. */
  @Input() maxTime: Moment | null;
   ...
   */
  get amPm(): string | null {
    return this.value ? (this.value.hour() < 12 ? "am" : "pm") : null;
  }
  set amPm(value: string) {
    if (this.timeUnitSelectComponents) {
      this.timeUnitSelectComponents
        .toArray()
  ...
  constructor(
    ...
    const time =
      this._timeAdapter.isDateInstance(data.value) &&
      this._timeAdapter.isValid(data.value as D)
        ? this._timeAdapter.clone(data.value as D)
        : this._timeAdapter.now();
    const value = this._timeAdapter.toMoment(
      this._timeAdapter.clampTime(time, data.minTime, data.maxTime),
    );
    ...
    this.minTime = data.minTime
      ? this._timeAdapter.toMoment(data.minTime)
      : null;
    this.maxTime = data.maxTime
      ? this._timeAdapter.toMoment(data.maxTime)
      : null;
  }
  ...
  getPrev(unit: unitOfTime.All): Moment {
    const prev = this.value
      .clone()
      .subtract(1 as DurationInputArg1, unit as DurationInputArg2);
  ...
  getNext(unit: unitOfTime.All): Moment {
    const next = this.value
      .clone()
      .add(1 as DurationInputArg1, unit as DurationInputArg2);
...
export class MatTimeSelectComponent<D> implements OnDestroy, CanColor {
  ...
  private _popupComponentPortal: ComponentPortal<
    MatTimeSelectContentComponent<D>
  >;
  ...
  get startAt(): D | null {
    return (
      this._startAt ||
      (this._timeSelectInput ? this._timeSelectInput.value : null)
    );
  }
  ...
  get color(): ThemePalette {
    return (
      this._color ||
      (this._timeSelectInput
        ? this._timeSelectInput.getThemePalette()
        : undefined)
    );
  }
  ...
  get disabled(): boolean {
    return this._disabled !== undefined
      ? this._disabled
      : this._timeSelectInput
        ? this._timeSelectInput.disabled
        : false;
  }
  ...
  constructor(
    ...
    @Inject(MAT_TIME_SELECT_SCROLL_STRATEGY)
  ...
  registerInput(input: MatTimeSelectInputDirective<D>): void {
    if (this._timeSelectInput) {
      throw Error(
        "A MatTimeSelect can only be associated with a single input.",
      );
    }
    this._timeSelectInput = input;
    this._inputSubscription = this._timeSelectInput.valueChange.subscribe(
  ...
  open(): void {
    ...
    if (!this._timeSelectInput) {
      throw Error(
        "Attempted to open an MatTimeSelect with no associated input.",
      );
    }
    if (this._document) {
  ...
  private _openAsPopup(): void {
    ...
    if (!this._popupRef.hasAttached()) {
      this._popupComponentRef = this._popupRef.attach(
        this._popupComponentPortal,
      );
      this._popupComponentRef.instance.valueChange.subscribe((value: Moment) =>
        this.select(this._timeAdapter.deserialize(value.toISOString())),
      );
      this._setColor();
   ...
   */
  private _getValidDateOrNull(obj: D | null): D | null {
    return this._timeAdapter.isDateInstance(obj) &&
      this._timeAdapter.isValid(obj as D)
      ? obj
      : null;
  }
